...
- **Play along with 200 real songs, sorted by difficulty.** Every genre has
  ...
  *Take Five*). The **"Song to play along"** list always shows the current
  ...
  generated by `tools/generate_songs.js` into `songs.js`. Each song gets a
  distinct groove, and ~25 iconic songs (Billie Jean, We Will Rock You, Back in
  Black, Funky Drummer, Superstition, Stayin' Alive, …) use their **actual
  signature drum beat** (drum rhythms aren't copyrightable). Those are labelled
  "Signature drum beat" in the app.
